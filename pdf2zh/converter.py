...
class Paragraph:
    ...
    def __init__(self, y, x, x0, x1, y0, y1, size, brk, ptexts):
...
class TranslateConverter(PDFConverterEx):
    ...
    def receive_layout(self, ltpage: LTPage):
        ...
        for child in ltpage:
            if isinstance(child, LTChar):
                ...
                if not cur_v:                                               # 文字入栈
                    ...
                    pstk[-1].ptexts.append(child.get_text())
                else:                                                       # 公式入栈
                    if (                                                    # 根据公式左侧的文字修正公式的纵向偏移
                        ...
                        vfix = child.y0 - xt.y0
                    vstk.append(child)
                    pstk[-1].ptexts.append(child.x1-child.x0)
        ...
        def calculate_text_height(text: str, size: float, x0: float, x1: float, line_spacing: float) -> float:
            ...
            for char in text:
                # 简化的字符宽度计算
                char_width = self.fontmap[fcur].char_lengths(char, size)
        ...
        def find_optimal_line_spacing(text: list, size: float, x0: float, x1: float,
            ...
            while current_spacing > min_spacing:
                new_height = calculate_text_height(text, size, x0, x1, current_spacing)
                if new_height <= original_height:
                    break
                current_spacing -= 0.1

            return max(current_spacing, min_spacing), max(original_height, new_height)
        ...
        for id, new in enumerate(news):
            ...
            print("new", new)
            # 使用目标语言的默认行距作为最大行距
            optimal_spacing, new_height = find_optimal_line_spacing(
                new, size, x0, x1, original_height,
                max_spacing=default_line_spacing
            )
            print("new_height", new_height)
            print("optimal_spacing", optimal_spacing)
            print("-" * 10)
